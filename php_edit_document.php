                ...
                ?>   
                    ...
                    <table border="1px solid silver" cellpadding="15px" cellspacing="10px"
                        ...
                        $result=mysql_query("select * from documents where document_id = '$_GET[docuid]'");
           ...
           // $EndDate=$_POST['EndDate'];
            ...
            // $coursefn=$_POST['cnf'];
            $document_id=$_GET['id'];
            $query="update documents set Name ='$Name' where document_id='$document_id'";
            $stmt = $mysqli->prepare($query);
            //$rc=$stmt->bind_param('TPP',$Title,$Project_introduction,$ProjectCode);
            $stmt->execute();
            echo"<script>alert('Document has been Updated successfully');</script>";
            }

            ?>
            <?php	
            $id=$_GET['id'];
            $ret="select * from  documents where document_id='$id'";
            $stmt= $mysqli->prepare($ret) ;
            // $stmt->bind_param($id);
            $stmt->execute() ;//ok
            $res=$stmt->get_result();
           ...
           <div class="form-group">
            ...
            <input type="text"  name="document_id"  value="<?php echo $row->document_id;?>" disabled class="form-control" > 
            <font color="red">Document Id Can not be Edited!!</font> 
            <p></p>
            <label class="col-sm-2 control-label">Documment Title&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <input type="text"  name="Document_Title"  value="<?php echo $row->Document_Title;?>" disabled class="form-control" > 
            <font color="red">Document Title  Can not be Edited!!</font>
            </div>
            <p></p>
            </div>
            <div class="form-group">
            <label class="col-sm-2 control-label">Document Body</label>
            <div class="col-sm-8">
            <textarea  class="form-control" name="Name" id="hada" value="<?php echo $row->Name;?>
            ...
            <input class="btn btn-primary" type="submit" name="submiting" value="Update Document">
            </form>
                        <div class="clr"></div>
                        <ul class="sb_menu">
